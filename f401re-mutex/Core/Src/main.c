...
TaskHandle_t handle_task_button;

SemaphoreHandle_t handle_count_semap_1;

EventGroupHandle_t handle_event_1;
SemaphoreHandle_t 	handle_mutex_1;
...
int main(void)
{
  ...
  //handle_mutex_1 == NULL;
  handle_mutex_1 = xSemaphoreCreateMutex();
  ...
  /* add threads, ... */
  xTaskCreate(task_main, "Main task", 64, NULL, osPriorityNormal, &handle_task_main);
  //assert_param(status == pdPASS);

  xTaskCreate(task_green, "Blinking green led task", 128, NULL, osPriorityNormal3, &handle_task_green);
  //assert_param(status == pdPASS);

  xTaskCreate(task_blue, "Blinking blue led task", 128, NULL, osPriorityNormal2, &handle_task_blue);
  //assert_param(status == pdPASS);

  xTaskCreate(task_red, "Blinking red led task", 64, NULL, osPriorityNormal1, &handle_task_red);
  //assert_param(status == pdPASS);

  xTaskCreate(button_task_read_entry, "Button task", 128, (void *)&button, osPriorityNormal, &handle_task_button);

  /* USER CODE END RTOS_THREADS */

  /* USER CODE BEGIN RTOS_EVENTS */
  /* add events, ... */
  handle_event_1 = xEventGroupCreate();
