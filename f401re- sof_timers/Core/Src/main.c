...
TimerHandle_t handle_timer_user;
...
int main(void)
{
  ...
  oneShotHandle = xTimerCreate("One shoot timer", pdMS_TO_TICKS(2200), pdFALSE, NULL, oneShotCallBack);
  xTimerStart(oneShotHandle,0); //Es posible usarlo acá, pero empezará luego de empezar el scheduler del freertos

  repeatHandle = xTimerCreate("Repeated timer", pdMS_TO_TICKS(500), pdTRUE, NULL, repeatCallBack);
  xTimerStart(repeatHandle,0);

  HAL_GPIO_WritePin(LED_RED_GPIO_Port, LED_RED_Pin, GPIO_PIN_SET);
  handle_timer_user = xTimerCreate("User timer", pdMS_TO_TICKS(1000), pdFALSE, NULL, timer_user_callback);
  xTimerStart(handle_timer_user,pdMS_TO_TICKS(1000));
